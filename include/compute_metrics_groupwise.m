 for idx=1:length(n_metrics_standard)
    ...
    if find(n_missing_metrics==mt)
        ...
        if contains(metrics{mt}.comparison_type,'scanpath')
            metrics_gazewise{mt}.score=metrics{mt}.score_submetric;
        end
    else
        %already calculated 
    end
    if contains(metrics{mt}.name,'AUC')>0 && ~isempty(find(n_missing_roc==mt)) && isempty(find(n_missing_metrics==mt)) %metric found but roc missing
